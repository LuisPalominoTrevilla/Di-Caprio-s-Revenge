...
public class FinalLevel implements Screen {
    ...
    //Constructor begins
    public FinalLevel(DiCaprio game, boolean TwoPlayers){
        ...
        map = maploader.load("Revenant.tmx");
        ...
        if(TwoPlayers){
            ...
            hud2 = new Hud(game.batch, player2, true, 6);
        }

        bearBoss = new BearBoss(world, 14800 / DiCaprio.PPM, 1000/DiCaprio.PPM);

        hud = new Hud(game.batch, player, false, 6);
        //set the sound
    ...
    public void update(float delta){
        ...
        bearBoss.update(delta);
        if(!bearBoss.isDestroyed() && (bearBoss.getX() < player.getX() + 300/DiCaprio.PPM)){
            bearBoss.b2body.setActive(true);
    ...
    public void render(float delta) {
        ...
        if (player.isNextLevel()) {
            game.setScreen(new LoadingScreen(6,game, TwoPlayers));
        }
        if(TwoPlayers){
